...
class VersionSolver:
    ...
    def _resolve_conflict(
        ...
        # is_failure() 是在檢查incompatibility是否只包含一個root package或是沒有包含任何term
        # 上述兩種發生任何一種表示 resolve failure 再次提醒 incompatibility 是不可變的
    ...
    def _choose_package_version(self):  # type: () -> Union[Hashable, None]
        ...
        version = versions[0]
        conflict = False
        # incompatibilities_for 去獲得相關的incompatibilities
